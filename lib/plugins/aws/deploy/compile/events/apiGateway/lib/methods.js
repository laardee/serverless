        ...
        if (event.http) {
          ...
          if (Boolean(event.http.response) === true) {
            if (typeof event.http.response === 'object') {
              // set default statuscode for response
              if (Boolean(event.http.response.statusCode) === true) {
                if (event.http.response.statusCode >= 200 && event.http.response.statusCode < 400) {
                  responseStatusCode = event.http.response.statusCode;
                }
              }
